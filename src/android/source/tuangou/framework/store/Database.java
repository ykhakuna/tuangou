...
public class Database {
	...
	public Database(Context context, String s) {
		...
		// 创建数据库帮助类对象
		dbOpenHelper = database.new DatabaseOpenHelper(context1, s, null, i);
	...
	public boolean execSql(String s) {
		...
		try {
			...
			String s1 = (new StringBuilder()).append("Failed to exec sql: ")
					.append(s).toString();
			LogUtil.e(sqlE, s1);
	...
	public Object[][] query(String sql, String selectionArgs[]) {
		...
		try {
			...
			/*
			 * rawQuery()方法用于执行select语句 参数1--select语句 参数2--select语句中占位符参数的值
			 */
			...
			String s2 = (new StringBuilder()).append("Failed to query sql: ")
					.append(sql).toString();
			LogUtil.e(e, s2);
			// TODO: handle exception
		}
